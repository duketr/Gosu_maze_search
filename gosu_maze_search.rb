...
class GameWindow < Gosu::Window
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      # boundary side
	  if @columns[cell_x][cell_y].east != nil && @columns[cell_x][cell_y].east.vacant && !@columns[cell_x][cell_y].east.visited
      @columns[cell_x][cell_y].east.visited = true
      east_path = search(cell_x + 1, cell_y)
    end

    if @columns[cell_x][cell_y].west != nil && @columns[cell_x][cell_y].west.vacant && !@columns[cell_x][cell_y].west.visited
      @columns[cell_x][cell_y].west.visited = true
      west_path = search(cell_x - 1, cell_y)
    end

    if @columns[cell_x][cell_y].south != nil && @columns[cell_x][cell_y].south.vacant && !@columns[cell_x][cell_y].south.visited
      @columns[cell_x][cell_y].south.visited = true
      south_path = search(cell_x, cell_y + 1)
    end

    if @columns[cell_x][cell_y].north != nil && @columns[cell_x][cell_y].north.vacant && !@columns[cell_x][cell_y].north.visited
